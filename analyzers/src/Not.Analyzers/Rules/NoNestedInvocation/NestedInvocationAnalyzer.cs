...
public class NestedInvocationAnalyzer : AnalyzerBase
{
    ...
    private const string DESCRIPTION = "Method invocations should be separated into multiple statements for better readability.";
    ...
    protected override void SafeAnalyzeSyntaxNode(SyntaxNodeAnalysisContext context)
    {
        ...
        {
            if (argument.Expression is InvocationExpressionSyntax ||
                argument.Expression.DescendantNodes().OfType<InvocationExpressionSyntax>().Any())
            {
                context.ReportDiagnostic(
                    CreateDiagnostic(invocation.GetLocation()));
                break;
            }
        }
    }
